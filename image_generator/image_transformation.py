...
def center_to_shape(image, shape, background_color=0):
    ...
    min_row = min(image.shape[0], shape[0])
    min_cols = min(image.shape[1], shape[1])
    ...
    res_box = np.array([center_result[0] - min_row // 2, 0,
                        center_result[1] - min_cols // 2, 0])
    res_box[1::2] = [res_box[0] + min_row, res_box[2] + min_cols]

    center_image = (image.shape[0] // 2, image.shape[1] // 2)
    img_box = np.array([center_image[0] - min_row // 2, 0,
                        center_image[1] - min_cols // 2, 0])
    img_box[1::2] = [img_box[0] + min_row, img_box[2] + min_cols]
